...
for ($attempt = 1; $attempt -le 30; $attempt++) {
    try {
        $response = Invoke-WebRequest -Uri 'http://localhost:8080/up' -UseBasicParsing -TimeoutSec 2
...
if (-not $ready) {
    ...
    throw 'Website chưa sẵn sàng tại http://localhost:8080. Kiểm tra log Docker trước khi mở tunnel.'
}
...
Write-Host ''
Write-Host 'Website nội bộ: http://localhost:8080' -ForegroundColor Green
