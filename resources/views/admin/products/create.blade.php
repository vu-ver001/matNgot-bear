...
<form id="create-product-form" onsubmit="handleStoreProduct(event)" enctype="multipart/form-data">
    @csrf
        ...
        <div>
            ...
            <div class="form-card">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="prod-name" name="name" class="input-control" required placeholder="Ví dụ: Gấu Bông Teddy Áo Len Cổ Điển...">
        ...
        grid.innerHTML = generalFiles.map((item, idx) => `
            <div class="preview-card ${item.is_primary ? 'is-primary' : ''}">
                ...
                <div class="preview-actions">
                    ...
                    <button type="button" class="btn-del-thumb" onclick="removeGeneralImage(${idx})" title="${item.sourceVariantUid ? 'Ảnh của sản phẩm con (không thể xóa tại đây)' : 'Xóa ảnh'}" style="${item.sourceVariantUid ? 'background: rgba(121, 85, 72, 0.9);' : ''}">
        ...
        tbody.innerHTML = variantsList.map((v, idx) => {
            ...
            return `
                <tr class="${v.is_default ? 'is-default-row' : ''}">
                    ...
                    <td>
                        <input type="number" class="v-input v-input-num" value="${formattedStock}" min="0" placeholder="20" oninput="updateVariantField(${idx}, 'stock_quantity', this.value)" required>
                    </td>
                    <td style="text-align: center;">
            ...
            if (v.sale_start_at && v.sale_end_at) {
                ...
                if (start < nowBuffer) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Bé Gấu nhắc bạn nè! 🐻',
                        html: `Ngày bắt đầu sale của <b>${label}</b> phải từ thời điểm hiện tại trở đi nha!`,
                        confirmButtonColor: '#8D6E63'
                    });
                    return;
                }
