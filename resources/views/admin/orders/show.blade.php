...
@section('content')
<div x-data="{ openApproveModal: false, openRejectModal: false, openRefundConfirmModal: false, openRejectOrderModal: false, rejectReason: '' }">
            ...
            <div class="flex items-start sm:items-center gap-3.5">
                ...
                <div>
                    <div class="flex items-center gap-2 flex-wrap">
                        <h3 class="text-base font-extrabold text-[#2B1810]">Đơn hàng mới - Chờ duyệt & xác nhận</h3>
                    ...
                    <p class="text-xs text-[#7D6B5D] mt-0.5">
                        Kiểm tra số lượng tồn kho và thông tin đơn hàng. Bấm <strong>Xác nhận</strong> để chuẩn bị đơn, hoặc <strong>Từ chối đơn</strong> (kèm lý do) nếu không thể tiếp nhận.
                    </p>
                </div>
            </div>
    ...
    {{-- POPUP MODAL TỪ CHỐI ĐƠN HÀNG (PENDING) DÀNH CHO ADMIN --}}
    @if ($order->order_status === 'PENDING')
                            ...
                            <div class="space-y-3.5 text-xs text-[#5C3219]">
                                ...
                                @if($order->payment_status === 'PAID')
                                    <div class="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-900 flex items-start gap-2">
                                        ...
                                        <span>Đơn hàng đã thanh toán <strong>{{ number_format($order->total_amount, 0, ',', '.') }}đ</strong>. Sau khi từ chối, hệ thống sẽ tự động tạo yêu cầu hoàn tiền để đối soát chuyển trả lại cho khách.</span>
                                    </div>
                                @endif
                            </div>
                        </div>
    ...
    @if($order->needsRefund())
        <div class="mb-6 bg-gradient-to-r from-amber-500/15 via-amber-50 to-orange-500/10 border-2 border-amber-400 rounded-2xl p-6 shadow-sm">
            ...
            <div class="mt-4 p-3.5 bg-white rounded-xl border border-amber-200 text-xs" x-data="{ openEditBankModal: false }">
                <div class="flex items-center justify-between flex-wrap gap-2 mb-2 pb-2 border-b border-gray-100">
                    ...
                    <button type="button" @click="openEditBankModal = true" class="text-amber-800 font-bold hover:underline flex items-center gap-1 cursor-pointer">
                ...
                {{-- MODAL NHẬP STK KHÁCH DÀNH CHO ADMIN --}}
                <div x-show="openEditBankModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
                    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        <div class="fixed inset-0 bg-black/60 backdrop-blur-xs transition-opacity" @click="openEditBankModal = false"></div>
                        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                        <div class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-amber-300" @click.stop>
                            <form method="POST" action="{{ route('admin.orders.update_refund_account', $order) }}">
                                @csrf
                                <div class="p-6">
                                ...
                                <div class="bg-gray-50 px-6 py-3.5 flex justify-end gap-2 border-t border-gray-100">
                                    <button type="button" @click="openEditBankModal = false" class="px-3.5 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-100 rounded-xl cursor-pointer">Đóng</button>
